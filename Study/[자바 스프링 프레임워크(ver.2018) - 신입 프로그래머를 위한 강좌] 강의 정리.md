...
데이터베이스에 여러 사람들이 접근할때

커넥션풀 : 데이터베이스 커넥션을 미리 만들어 놓는 곳

커넥션 풀 만드는 방법1 :c3p0모듈의 ComboPooledDataSource 

커넥션 풀 만드는 방법2 : 스프링 설정파일을 이용한 DataSource 설정

커넥션 풀은 예외처리가 필요하다.

커넥션이 데이터 접근에서 제일 널리 사용되는 방법이다.